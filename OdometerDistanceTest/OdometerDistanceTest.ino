/*
 * Test to check that the robot is able to determine accurate travel distance. It's just the
 * normal code, but the waypoint is a meter off.
 */
...
const uint16_t UPPER_THRESHOLD = 150; //thresholds used for the odometer
const uint16_t LOWER_THRESHOLD = 50;

const float WHEEL_CIRC_CM = 14.11;
...
JVector waypoints[] = {JVector(0,0), JVector(0,1)};
...
void control(){
  if(killSwitch()){
    ...
    motorForward();
    if(halfTurns >= 100.0 / (WHEEL_CIRC_CM / 2)){
      steerStraight();
      motorStop();
      while(digitalRead(BUTTON) == LOW){
      }
