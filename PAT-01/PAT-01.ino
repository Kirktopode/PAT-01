...
const int RTRN = A3; //Right turn pin --if both turn pins are HIGH, no turn is made.
...
void navigate() {
  ...
  epochTime = millis();
  intervalTime = epochTime - lastTime;
...
const int SONAR_BUFF_LEN = 5;

int cmpfunc (const void * a, const void * b) {
   return ( *(int*)a - *(int*)b );
}

int getMedian(unsigned long* ar){
  qsort(ar, SONAR_BUFF_LEN, sizeof(unsigned long), cmpfunc);
  return ar[SONAR_BUFF_LEN / 2];
}
...
unsigned long sonarBuffLeft[SONAR_BUFF_LEN];
unsigned long sonarBuffCenter[SONAR_BUFF_LEN];
unsigned long sonarBuffRight[SONAR_BUFF_LEN];

void detect() {
  
  sonarBuffLeft[sbi] = sonarLeft.ping_cm();
  sonarBuffCenter[sbi] = sonarCenter.ping_cm();
  sonarBuffRight[sbi] = sonarRight.ping_cm();
  sbi++;
  if(sbi == SONAR_BUFF_LEN){
    leftDist = getMedian(sonarBuffLeft);
    centerDist = getMedian(sonarBuffCenter);
    rightDist = getMedian(sonarBuffRight);
    sbi = 0;
  }
  Serial.print(",");
...
void control() {
  if (killSwitch()) {
    ...
    if (headingChange < -900) {
      ...
      motorBack();
      timeSpentBacking += intervalTime;
      Serial.println("Backing Up");
      if (timeSpentBacking >= BACKUP_TIME){
        backingUp = false;
        timeSpentBacking = 0;
        steerStraight();
      }
    ...
    } else if (centerDist < BACKUP_DIST && centerDist != 0) {
      motorBack();
      steerLeft();
      backingUp = true;
